...
export default function PopupWindow({className, children, isPopupAbsolute, closeWindow}) {
		...
		<div className={cn(s.window, popupType, className)}>
			<div className={s.window__container}>
				<button className={cn(s.window__close, 'icon_close1')}
					onClick={() => closeWindow ? dispatch(closeWindow(false)) : dispatch(showWindowInfo(false))}
					></button>
